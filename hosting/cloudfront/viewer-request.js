...
// at the edge (ADR-0010, ADR-0011, docs/implementation/07-hosting-and-go-live.md step 07a.1). Deploys publish this
// file unchanged to each environment's function.
//
// - www.kroiko.com is not an origin of its own (ADR-0011): a request for www.<domain> gets a 301 to the same path
//   on https://<domain>. The query string is dropped; the app uses none.
//
...
function handler(event) {
    ...
    if (host.indexOf('www.') === 0) {
        return {
            ...
            headers: { location: { value: 'https://' + host.substring('www.'.length) + request.uri } },
        };
    }
